    ...
    constructor(ctx, chartType) {
        ...
        function seriesInfoDiv(label, color, value, units, trackDecimals, active, percent, valueFormatFunction) {
            ...
            if (valueFormatFunction) {
                ...
                valueContent = iotopFlot.ctx.utils.formatValue(value, trackDecimals, units);
            }
        ...
        function seriesInfoDivFromInfo(seriesHoverInfo, seriesIndex) {
            var units = seriesHoverInfo.units && seriesHoverInfo.units.length ? seriesHoverInfo.units : iotopFlot.ctx.trackUnits;
            var decimals = angular.isDefined(seriesHoverInfo.decimals) ? seriesHoverInfo.decimals : iotopFlot.ctx.trackDecimals;
        ...
        if (this.chartType === 'pie') {
            ctx.tooltipFormatter = function(item) {
                var units = item.series.dataKey.units && item.series.dataKey.units.length ? item.series.dataKey.units : iotopFlot.ctx.trackUnits;
                var decimals = angular.isDefined(item.series.dataKey.decimals) ? item.series.dataKey.decimals : iotopFlot.ctx.trackDecimals;
            ...
            ctx.tooltipFormatter = function(hoverInfo, seriesIndex) {
                ...
                if (iotopFlot.ctx.tooltipIndividual) {
    ...
    enableMouseEvents() {
        ...
        if (!this.flotHoverHandler) {
            this.flotHoverHandler =  function (event, pos, item) {
                if (!iotopFlot.ctx.plot) {
                    return;
                }
                if (!iotopFlot.ctx.tooltipIndividual || item) {

                    var multipleModeTooltip = !iotopFlot.ctx.tooltipIndividual;

                    if (multipleModeTooltip) {
                        iotopFlot.ctx.plot.unhighlight();
                    }
                    ...
                    if (iotopFlot.chartType === 'pie') {
                        tooltipHtml = iotopFlot.ctx.tooltipFormatter(item);
                    } else {
                        var hoverInfo = iotopFlot.getHoverInfo(iotopFlot.ctx.plot.getData(), pos);
                        if (angular.isNumber(hoverInfo[0].time) || (hoverInfo[1] && angular.isNumber(hoverInfo[1].time))) {
                            ...
                            tooltipHtml = iotopFlot.ctx.tooltipFormatter(hoverInfo, item ? item.seriesIndex : -1);
                        }
                    }

                    if (tooltipHtml) {
                        iotopFlot.ctx.tooltip.html(tooltipHtml)
                            .css({top: 0, left: 0})
                            .fadeIn(200);

                        var windowWidth = $( window ).width();  //eslint-disable-line
                        var windowHeight = $( window ).height();  //eslint-disable-line
                        var tooltipWidth = iotopFlot.ctx.tooltip.width();
                        var tooltipHeight = iotopFlot.ctx.tooltip.height();
                        var left = pageX+5;
                        var top = pageY+5;
                        ...
                        if (multipleModeTooltip) {
                            for (var j = 0; j < hoverInfo.length; j++) {
                                for (var i = 0; i < hoverInfo[j].seriesHover.length; i++) {
                                    ...
                                    iotopFlot.ctx.plot.highlight(seriesHoverInfo.index, seriesHoverInfo.hoverIndex);
                                }
                            }
                        }
                    }
                    ...
                    iotopFlot.ctx.tooltip.stop(true);
                    iotopFlot.ctx.tooltip.hide();
                    iotopFlot.ctx.plot.unhighlight();
                }
            };
        ...
        if (!this.flotClickHandler) {
            this.flotClickHandler =  function (event, pos, item) {
                ...
                iotopFlot.onPieSliceClick(event, item);
            };
